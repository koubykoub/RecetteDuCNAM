...
<form method="post" action="" name="modifier_recette" enctype="multipart/form-data">
	...
	<fieldset>
		...
		<?php if ($dVueCreerRecette['mise_a_jour'] && isset($dVueCreerRecette['recette']['photo']) && is_string($dVueCreerRecette['recette']['photo'])) { ?>
		<br /><label for="effacer_image">Effacer l'image actuelle &nbsp;</label><input type="checkbox" name="effacer_image" id="effacer_image" value="true" /><br />
		<img alt="" src="?page=image&image=<?php echo $dVueCreerRecette['recette']['photo']; ?>" />
		<?php } ?>
	</fieldset>
